					    ...
					    size_t rc_len)
{
	...
	lens = *pos++;
	wpa_printf(MSG_DEBUG, "hanfushun: %s   ie = %x%x%x  cred = %x%x%x", __func__, *pos,*(pos+1),*(pos+2), *rc_id, *(rc_id+1), *(rc_id+2));
	...
	int *excluded)
{
	...
	wpa_printf(MSG_DEBUG, "hanfushun: selected = %d", selected!=NULL);
	return selected;
}
